...
DallasTemperature::DallasTemperature(DOneWire *_oneWire) : DallasTemperature()
{
	setOneWire(_oneWire);
}
 ...
 */
DallasTemperature::DallasTemperature(DOneWire *_oneWire, uint8_t _pullupPin) : DallasTemperature(_oneWire)
{
...
void DallasTemperature::setOneWire(DOneWire *_oneWire)
{
												...
												uint8_t *scratchPad)
{
	...
	if (deviceAddress[DSROM_FAMILY] == DS1825MODEL && scratchPad[CONFIGURATION] & 0x80)
	{
		...
		fpTemperature = (((int32_t)scratchPad[TEMP_MSB]) << 11) | (((int32_t)scratchPad[TEMP_LSB] & 0xFC) << 3) | neg;
	}
	else
	{
		fpTemperature = (((int16_t)scratchPad[TEMP_MSB]) << 11) | (((int16_t)scratchPad[TEMP_LSB]) << 3) | neg;
	}
